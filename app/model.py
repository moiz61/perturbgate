...
def load_model_bundle(path: Path) -> LoadedModel:
    ...
    if "model" not in bundle:
        raise ValueError("Model bundle is missing 'model'.")

    if "feature_names" not in bundle:
        raise ValueError("Model bundle is missing 'feature_names'.")

    feature_names = tuple(bundle["feature_names"])

    if feature_names != EXPECTED_FEATURE_NAMES:
        raise ValueError(
            "Model feature order does not match the API feature order.\n"
    ...
    metrics = {
        key: float(value)
        for key, value in bundle.get("metrics", {}).items()
    }

    return LoadedModel(
        model=model,
        feature_names=feature_names,
        model_sha256=calculate_sha256(path),
        metrics=metrics,
    )
...
def predict_probabilities(
    ...
    if matrix.shape[1] != len(EXPECTED_FEATURE_NAMES):
        raise ValueError(
            f"Expected {len(EXPECTED_FEATURE_NAMES)} features, "
            f"received {matrix.shape[1]}."
        )

    probabilities = loaded_model.model.predict_proba(matrix)[:, 1]
